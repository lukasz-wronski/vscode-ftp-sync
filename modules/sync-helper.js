...
var listLocalFiles = function(localPath, callback, options) {
  ...
  fswalk.walk(
    ...
    function(basedir, filename, stat, next) {
      ...
      if (isIgnored(filePath, ftpConfig.allow, ftpConfig.ignore)) return next();
...
var prepareSyncObject = function(remoteFiles, localFiles, options, callback) {
  ...
  callback(null, {
    _readMe:
      'Review list of sync operations, then use Ftp-sync: Commit command to accept changes',
...
var executeSyncLocal = function(sync, options, callback) {
  ...
  if (sync.dirsToAdd.length > 0) {
    ...
    let localPath = path.join(options.localPath, fileToRemove);
    ...
    let localPath = path.join(options.localPath, dirToRemove);
...
var executeSyncRemote = function(sync, options, callback) {
  ...
  if (sync.dirsToAdd.length > 0) {
    ...
    var fileToRemove = sync.filesToRemove.pop();
    let remotePath = upath.toUnix(path.join(options.remotePath, fileToRemove));
    ...
    var dirToRemove = sync.dirsToRemove.pop();
    let remotePath = upath.toUnix(path.join(options.remotePath, dirToRemove));
...
var ensureDirExists = function(remoteDir, callback) {
  ftp.list(path.posix.join(remoteDir, '..'), function(err, list) {
    ...
    else if (_.any(list, f => f.name == path.basename(remoteDir))) callback();
    else
...
const listOnceRemoteFiles = function(remotePath, callback) {
  output(getCurrentTime() + ' > [ftp-sync] listRemoteFiles: ' + remotePath);
  remotePath = upath.toUnix(remotePath);
  ftp.list(remotePath, function(err, remoteFiles) {
    // The request is finish so remove it
    ...
    remoteFiles.forEach(function(fileInfo) {
      ...
      if (fileInfo.name == '.' || fileInfo.name == '..') return fileInfo;
      var remoteItemPath = upath.toUnix(path.join(remotePath, fileInfo.name));
      if (fileInfo.type != 'd')
        result.push({
          name: remoteItemPath,
...
const toListRemoteFiles = function name(remotePath, callback) {
  connect(function(err) {
    if (err) {
      callback(err);
      return;
    }
    listOnceRemoteFiles(remotePath, callback);
  });
};
...
var helper = {
  ...
  toListRemoteFiles: toListRemoteFiles,
