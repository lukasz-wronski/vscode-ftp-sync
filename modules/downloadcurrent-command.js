...
module.exports = function(fileUrl, getFtpSync) {
  ...
  var downloadStatus = vscode.window.setStatusBarMessage(
    'Ftp-sync: Downloading ' + fileName + ' from FTP server...',
    STATUS_TIMEOUT
  );
  getFtpSync().downloadFile(fileUrl.fsPath, vscode.workspace.rootPath, function(
    err
  ) {
    downloadStatus.dispose();
    if (err)
      vscode.window.showErrorMessage(
        'Ftp-sync: Downloading ' + fileName + ' failed: ' + err
      );
    else
      vscode.window.setStatusBarMessage(
        'Ftp-sync: ' + fileName + ' downloaded successfully!',
        STATUS_TIMEOUT
      );
  });
};
