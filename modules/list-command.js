...
module.exports = function (fileUrl, getFtpSync) {
	...
	let remotePath = getFatherPath(filePath.replace(ftpconfig.getLocalPath(), config.remotePath));
...
function getLocalPath(fileRemotePath) {
	return {
		fsPath: fileRemotePath.replace(ftpconfig.getConfig().remotePath, ftpconfig.getLocalPath())
	};
}
